...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  return (
    <>
      ...
      {activeItem && (
        <>
          <h2 className={`face-title ${isMovingRef.current ? 'dragging' : 'active'}`}>{activeItem.title}</h2>

          <p className={`face-description ${isMovingRef.current ? 'dragging' : 'active'}`}></p>

          <div onClick={handleButtonClick} className={`action-button ${isMovingRef.current ? 'dragging' : 'active'}`}>
